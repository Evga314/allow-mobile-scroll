...
public final class DragScrollConfigScreen extends Screen {
    ...
    private Button wheelRepositionButton;
    ...
    private int wheelRepositionRel;
    private int wheelRepositionDescRel;
    ...
    private boolean savedWheelReposition;
    ...
    public DragScrollConfigScreen(Screen parent) {
        ...
        this.savedWheelReposition = DragScrollConfig.isWheelRepositionEnabled();
    }
    ...
    protected void init() {
        ...
        // The Save/Reset/Cancel buttons are fixed in the upper-right now,
        // so the scrollable list no longer needs the old footer reserve.
        ...
        this.wheelRepositionRel = y;
        y += 24;
        this.wheelRepositionDescRel = y;
        y += this.wrappedHeight("dragscroll.config.wheel_reposition.description") + 8;
        ...
        this.wheelRepositionButton = this.addRenderableWidget(Button.builder(
                this.getWheelRepositionButtonText(),
                button -> {
                    DragScrollConfig.setWheelRepositionEnabled(!DragScrollConfig.isWheelRepositionEnabled());
                    this.updateWheelRepositionButton();
                }
        ).pos(this.fieldLeft, contentY(this.wheelRepositionRel)).size(this.fieldWidth, 20).build());
        ...
        // Keep the configuration actions outside the scrollable list.
        // They stay fixed in the upper-right corner while the settings list moves underneath.
        ...
        ).pos(buttonX, buttonY).size(buttonWidth, 20).build());
    ...
    private void repositionContent() {
        ...
        if (this.wheelRepositionButton != null) {
            this.wheelRepositionButton.setY(contentY(this.wheelRepositionRel));
        }
    ...
    private void updateContentVisibility() {
        ...
        boolean wrep = inView(this.wheelRepositionRel, 20);
        ...
        if (this.wheelRepositionButton != null) {
            this.wheelRepositionButton.visible = wrep;
            this.wheelRepositionButton.active = wrep;
        }
        if (this.debugButton != null) {
    ...
    public void extractRenderState(GuiGraphicsExtractor guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        drawWrapped(guiGraphics, "dragscroll.config.wheel_reposition.description", centerX, contentY(this.wheelRepositionDescRel), 0xFFA0A0A0);
    ...
    private Component getWheelRepositionButtonText() {
        return Component.translatable(DragScrollConfig.isWheelRepositionEnabled()
                ? "dragscroll.config.wheel_reposition.on"
                : "dragscroll.config.wheel_reposition.off");
    }

    private void updateWheelRepositionButton() {
        if (this.wheelRepositionButton != null) {
            this.wheelRepositionButton.setMessage(this.getWheelRepositionButtonText());
        }
    }
    ...
    private boolean isDirty() {
        ...
        return threshold != this.savedThreshold
                ...
                || DragScrollConfig.isWheelRepositionEnabled() != this.savedWheelReposition
    ...
    public void onClose() {
        ...
        DragScrollConfig.setWheelRepositionEnabled(this.savedWheelReposition);
